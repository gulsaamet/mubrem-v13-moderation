...
exports.run = async (client, message, args, embed) => {
    ...
    if (secim == "oluştur" || secim == "olustur") {
        let date = args[1];
        let name = args[2];
        if (!date) return message.reply({ content: `Saat ve dakika belirtilmedi, örnek kullanım: \`${ayarlar.prefix}alarm oluştur <17:30> <alarm ismi>\`` });
        let dateMatch = date.match(/\d\d:\d\d/);
        let hourMatch = date.match(/\d\d:/);
        let minuteMatch = date.match(/:\d\d/);
        if (!hourMatch || !minuteMatch) return message.reply({ content: `Saat ve dakika belirtilmedi, örnek kullanım: \`${ayarlar.prefix}alarm oluştur <17:30> <alarm ismi>\`` });
        hourMatch = hourMatch[0].slice(0, 2);
        minuteMatch = minuteMatch[0].slice(1);
        if (hourMatch > 24) return message.reply({ content: `Hatalı saat belirtildi, örnek kullanım: \`${ayarlar.prefix}alarm oluştur <17:30> <alarm ismi>\`` })
        if (minuteMatch > 60) return message.reply({ content: `Hatalı dakika belirtildi, örnek kullanım: \`${ayarlar.prefix}alarm oluştur <17:30> <alarm ismi>\`` })
        if (hourMatch.length > 0 && !hourMatch && isNaN(hourMatch)) return message.reply({ content: `Saat belirtilmedi, örnek kullanım: \`${ayarlar.prefix}alarm oluştur <17:00> <alarm ismi>\`` });
        if (minuteMatch.length > 0 && !minuteMatch && isNaN(minuteMatch)) return message.reply({ content: `Dakika belirtilmedi, örnek kullanım: \`${ayarlar.prefix}alarm oluştur <17:00> <alarm ismi>\`` });
        ...
        *Alarmların hakkında detaylı bilgi almak için; \`${ayarlar.prefix}alarm bilgi <id>\``)]
        })
    } else if (secim == "bilgi") {
        let id = args[1];
        ...
        let alarmData = await alarmDatabase.find({ memberID: message.author.id });
        message.reply({ content: `${alarmData.length > 0 && alarmData ? `${alarmData.map(alarmDat => `• Alarm id: \`${alarmDat.alarmID}\``).join("\n")}\n\nAlarmların hakkında detaylı bilgi almak için: \`${ayarlar.prefix}alarm bilgi <id>\`` : `Her hangi bir alarmınız bulunmuyor. Alarm oluşturmak için \`${ayarlar.prefix}alarm oluştur\``}` })
    }
